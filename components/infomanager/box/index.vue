<template>
	<div>
		...
		<ininfor-manager :productid="productid" columntitle="意见箱" specialnum="1"></ininfor-manager>
		</template>
	</div>
</template>
		...
		watch:{
			$route:{
				handler(){
					...
					productid && this.$router.push({path:'/infomanagerbox/'+productid});
				}
			},
			productid(){
			...
			init(fn){
				...
				var t = setInterval(() => {
					if(Vue.productList){
						...
						this.$router.push({path:'/infomanagerbox/'+productid});
